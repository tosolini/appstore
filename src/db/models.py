...
class ArcaneConfig(Base):
    """Arcane configuration"""
    __tablename__ = "arcane_config"
    
    id = Column(Integer, primary_key=True)
    base_url = Column(String(500), nullable=False)
    api_key_encrypted = Column(Text, nullable=False)  # Encrypted
    environment_id = Column(Integer, default=0)
    is_configured = Column(Boolean, default=False)
    force_mock_mode = Column(Boolean, default=False)
    last_validated = Column(DateTime, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    active_backend = Column(String(20), default="portainer")  # "portainer" | "arcane"
    ...
    def __repr__(self):
        return f"<ArcaneConfig(base_url={self.base_url}, configured={self.is_configured})>"
...
class DeployLog(Base):
    ...
    backend = Column(String(20), default="portainer")  # portainer, arcane
    portainer_stack_id = Column(Integer, nullable=True)
    arcane_project_id = Column(String(100), nullable=True)
...
class FavoriteApp(Base):
    """User's favorite apps"""
    __tablename__ = "favorite_apps"
